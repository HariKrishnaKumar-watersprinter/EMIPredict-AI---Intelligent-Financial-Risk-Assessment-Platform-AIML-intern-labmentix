...
class EMIMongoDBManager:
    ...
    # Map folder keys to their config collection names
    FOLDER_MAP = {
        1: MongoDBConfig.APPLICANTS_FOLDER_1,
        2: MongoDBConfig.APPLICANTS_FOLDER_2,
        3: MongoDBConfig.APPLICANTS_FOLDER_3
    }
    
    def __init__(self, connection_uri: str = None, db_name: str = None, folder: int = None):
        """
        Initialize MongoDB Manager.
        
        :param folder: 1, 2, or 3 to target a specific sub-folder. 
                       If None, targets the root 'applicants' collection.
        """
        ...
        logger.info(f"MongoDB connected: {self.db_name} | Target: {self.applicants.name}")
    ...
    def _setup_collections(self):
        """Setup collections based on the selected folder"""
        # Determine collection name
        if self.folder in self.FOLDER_MAP:
            collection_name = self.FOLDER_MAP[self.folder]
        else:
            collection_name = MongoDBConfig.APPLICANTS_COLLECTION
            
        self.applicants = self.db[collection_name]
        ...
        # Initialize counter for auto-increment ID (scoped to the specific collection)
        counter_id = f"applicant_id_{collection_name}"
        if self.db.counters.find_one({"_id": counter_id}) is None:
            self.db.counters.insert_one({"_id": counter_id, "seq": 0})
    
    def _create_indexes(self):
        """Create indexes optimized for your dataset queries"""
        logger.info(f"Creating indexes for {self.applicants.name}...")
    ...
    def _get_next_id(self) -> int:
        """Get auto-incrementing applicant ID scoped to the active collection"""
        counter_id = f"applicant_id_{self.applicants.name}"
    ...
    def _calculate_derived_fields(self, data: Dict) -> Dict:
        ...
        months_emergency = round(data.get('emergency_fund', 0) / total_obligations, 2) if total_obligations > 0 else 0
    ...
    def _log_operation(self, applicant_id: int, operation: str,
        ...
        try:
            self.audit_logs.insert_one({
                ...
                "collection": self.applicants.name,  # Track which folder/collection
                "operation": operation,
    ...
    # =========================================================================
    # CREATE OPERATIONS (Remains exactly the same, automatically applies to selected folder)
    # =========================================================================
    ...
    def create_applicant(self, applicant_data: Dict,
        ...
        for field, rules in FIELD_VALIDATION.items():
            if field in applicant_data and applicant_data[field] is not None:
                ...
                if expected_type:
                    is_float_type = (expected_type is float) or (isinstance(expected_type, tuple) and float in expected_type)
                    is_int_type = (expected_type is int) or (isinstance(expected_type, tuple) and int in expected_type)
                    
                    if is_float_type and isinstance(value, (int, float)):
                        applicant_data[field] = float(value)
                    elif is_int_type and isinstance(value, (int, float)):
                        applicant_data[field] = int(value)
        ...
        document = {
            ...
            **derived,
            ...
            "predicted_eligibility": applicant_data['Predicted_EMI_Eligibility'] if applicant_data.get('Predicted_EMI_Eligibility') else None,
            "predicted_probability":applicant_data['Confidence'] if applicant_data.get('Confidence') else None,
            "predicted_max_emi": applicant_data['Predicted_EMI_Amount'] if applicant_data.get('Predicted_EMI_Amount') else None,
            "risk_category": None,
        ...
        logger.info(f"Created applicant {applicant_id} in {self.applicants.name}")
        return self._serialize(document)

    # ... (Keep all your remaining READ, UPDATE, DELETE methods exactly as they are) ...
    # They will naturally apply to whatever folder collection is active.
    @handle_mongo_errors
